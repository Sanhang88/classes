...
tf_dtheta2az = tf( K_alpha2az*[Z_dlte M_dlte+Z_dlte*M_q],...
    [M_dlte M_dlte*Z_alpha-Z_dlte*M_alpha] );

%升降舵半损故障对应传递函数
tf_dlte2dtheta_half = -tf( [M_dlte_half M_dlte_half*Z_alpha-...
    Z_dlte_half*M_alpha],[1 Z_alpha+M_q M_alpha+Z_alpha*M_q] );
tf_dtheta2az_half = tf( K_alpha2az*[Z_dlte_half M_dlte_half+Z_dlte_half*...
    M_q],[M_dlte_half M_dlte_half*Z_alpha-Z_dlte_half*M_alpha] );

%升降舵全损故障对应传递函数
tf_dlte2dtheta_full = -tf( [M_dlte_full M_dlte_full*Z_alpha-Z_dlte_full*...
    M_alpha],[1 Z_alpha+M_q M_alpha+Z_alpha*M_q] );
tf_dtheta2az_full = tf( K_alpha2az*[Z_dlte_full M_dlte_full+Z_dlte_full*...
    M_q],[M_dlte_full M_dlte_full*Z_alpha-Z_dlte_full*M_alpha] );
...
for ii_kesi1 = kesi1_min:0.1:1
    for ii_wn1 = wn1_min:0.1:wn1_max
        ...
        array_kq_half(jj) = ( Z_alpha + M_q - 2*ii_kesi1*ii_wn1 )/...
            M_dlte_half;
        array_kaz_half(jj) = ( M_alpha+Z_alpha*M_q-M_dlte_half*...
            Z_alpha*array_kq_half(jj) - ii_wn1^2 )/( M_dlte_half*...
            K_alpha2az );
        array_kq_full(jj) = ( Z_alpha + M_q - 2*ii_kesi1*ii_wn1 )/...
            M_dlte_full;
        array_kaz_full(jj) = ( M_alpha+Z_alpha*M_q-M_dlte_full*...
            Z_alpha*array_kq_full(jj) - ii_wn1^2 )/( M_dlte_full*...
            K_alpha2az );
    end
end
...
for ii_kesi2 = kesi2_min:0.1:1
    for ii_wn2 = wn2_min:0.1:wn2_max
        ...
        array_kq2_half(jj) = ( Z_alpha + M_q - 2*ii_kesi2*ii_wn2 )/...
            M_dlte_half;
        array_kaz2_half(jj) = ( M_alpha+Z_alpha*M_q-M_dlte_half*...
            Z_alpha*array_kq2_half(jj) - ii_wn2^2 )/( M_dlte_half*...
            K_alpha2az );
        array_kq2_full(jj) = ( Z_alpha + M_q - 2*ii_kesi2*ii_wn2 )/...
            M_dlte_full;
        array_kaz2_full(jj) = ( M_alpha+Z_alpha*M_q-M_dlte_full*...
            Z_alpha*array_kq2_full(jj) - ii_wn2^2 )/( M_dlte_full*...
            K_alpha2az );
    end
end
